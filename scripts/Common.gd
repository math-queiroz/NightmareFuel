...
static var base_wrong_order_sanity_drain : float = 3.0
static var base_spill_sanity_drain : float = 0.1

static func get_avaliable_cups(cycle: int) -> int:
	return 1 if cycle < 2 else 2 if cycle < 5 else 3

static func get_avaliable_bottles(cycle: int) -> int:
	return 2 if cycle < 2 else 3 if cycle < 3 else 4

static func get_is_ice_avaliable(cycle: int) -> int:
	return cycle > 1
